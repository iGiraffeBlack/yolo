    ...
    while not queue.empty():
        ...
        for track in tracker.tracks:
            ...
            #save bounding box data
               ...
               gallery_path = image_path+'/gallery/'+str(track.track_id).zfill(4)
               ...
               if not cam_name == "cam1":
                   ...
                   if not os.path.isfile(file_path) and video_capture.get(cv2.CAP_PROP_POS_FRAMES) % 5 == 0:
               ...
               if cam_name == "cam1":    
                   dst_path = query_path 
                    #if file does not exist --> save
                   file_path = dst_path+'/'+str(track.track_id).zfill(4)+'_'+cam_seq+'_'+str(video_capture.get(cv2.CAP_PROP_POS_FRAMES))+'.png' 
                   if not os.path.isfile(file_path) and video_capture.get(cv2.CAP_PROP_POS_FRAMES) % 3 == 0 and video_capture.get(cv2.CAP_PROP_POS_FRAMES) < (780/2):
                       cv2.imwrite(file_path,frame2)#save cropped frame
                   elif video_capture.get(cv2.CAP_PROP_POS_FRAMES) % 3 == 0 and video_capture.get(cv2.CAP_PROP_POS_FRAMES) >= (780/2):
                       dst_path = image_path+'/bounding_box_test'
                       file_path = dst_path+'/'+str(track.track_id).zfill(4)+'_'+cam_seq+'_'+str(video_capture.get(cv2.CAP_PROP_POS_FRAMES))+'.png'
